...
export default function NewChallengePage() {
  ...
  const isAuthReady = hydrated && Boolean(streamer?._id);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (invalidTargets.length > 0) {
      setError('All goals must have a target limit of at least 1');
      setIsSubmitting(false);
      return;
    }
    ...
    try {
      if (!streamer?._id) {
        throw new Error('Streamer account not found');
      }
      ...
      await createChallenge({
        ...
        givenBy: formData.given_by || undefined,
        deadline: formData.deadline || undefined,
        rewardAmount: formData.reward_amount || undefined,
        subChallenges: validSubChallenges.map((sub) => ({
          title: sub.title,
              ...
              {subChallenges.map((subChallenge, index) => (
                <div key={subChallenge.id} className="bg-armor border border-gunmetal p-4 tactical-border space-y-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <label className="text-[10px] font-mono text-dimmed uppercase tracking-widest">Goal_Title</label>
                      <input
                        type="text"
          ...
          <section className="space-y-4">
            <div className="bg-armor border border-gunmetal p-4 md:p-6 tactical-border">
              <form onSubmit={handleSubmit}>
                <button
                  type="submit"
                  disabled={isSubmitting || !formData.title || !isAuthReady}
